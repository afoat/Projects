namespace Foat.Collections.Generic.Tests
{
    ...
    using Foat.Collections.Generic.Fakes;
    ...
    public partial class SkipListTests
    {
        ...
        public void InsertValue()
        {
            using (ShimsContext.Create())
            {
                ShimSkipList<int>.AllInstances.RandomLevel = (sl) => { return 1; };

                SkipList<int> skipList = new SkipList<int>();
                skipList.Insert(50);
                Assert.AreEqual<int>(50, skipList.Head.Next[0].Value);
                Assert.AreEqual<int>(1, skipList.Head.Next[0].Levels);
                Assert.AreEqual<int>(1, skipList.Head.Next[0].Levels);
                Assert.AreEqual<int>(1, skipList.Head.Next[0].Next.Length);
            }
        }
        ...
        [TestMethod]
        [TestCategory("Foat\\Collections\\Generic\\SkipList")]
        public void Contains_1LevelOneNode_Found()
        {
            Assert.IsTrue(this.OneLevelOneNode.Contains(50));
        }

        [TestMethod]
        [TestCategory("Foat\\Collections\\Generic\\SkipList")]
        public void Contains_1LevelOneNode_NotFound()
        {
            Assert.IsFalse(this.OneLevelOneNode.Contains(75));
        }
